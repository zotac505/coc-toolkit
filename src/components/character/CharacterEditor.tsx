...
import { CharacterData, RuleEdition, SkillItem } from '../../types/character';
    ...
    <div className="space-y-6 pb-20">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 bg-slate-900/80 p-4 rounded-xl border border-slate-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-100 flex items-center gap-2">
              <span>{char.name || '新しい探索者'}</span>
              <span className="text-xs px-2.5 py-0.5 rounded-full bg-emerald-950 border border-emerald-500/30 text-emerald-400">
                {char.edition === '6th' ? '第6版' : '第7版'}
              </span>
            </h2>
            <p className="text-xs text-slate-400">キャラクター作成 ＆ シート編集</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-2 px-3.5 py-2 rounded-lg bg-cyan-700 hover:bg-cyan-600 text-white text-sm font-medium transition shadow-lg shadow-cyan-950/40"
            title="ココフォリアの盤面へ直接貼り付け可能なJSONをコピー"
          >
            {copied ? <Check className="w-4 h-4 text-emerald-300" /> : <Copy className="w-4 h-4" />}
            <span>{copied ? 'コピー完了！' : 'ココフォリア駒出力'}</span>
          </button>
          ...
          <button
            onClick={() => onSave(char)}
            className="flex items-center gap-2 px-4 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white text-sm font-medium transition shadow-lg shadow-emerald-950/40"
          >
      ...
      <div className="bg-slate-900/60 p-5 rounded-xl border border-slate-800 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div>
            <label className="block text-xs text-slate-400 mb-1">探索者名</label>
            <input
              type="text"
              value={char.name}
          ...
          <div>
            <label className="block text-xs text-slate-400 mb-1">職業</label>
            <input
              type="text"
              value={char.job}
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="block text-xs text-slate-400 mb-1">年齢</label>
              <input
                type="text"
                value={char.age}
        ...
        <div className="lg:col-span-2 bg-slate-900/60 p-5 rounded-xl border border-slate-800 space-y-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {(['str', 'con', 'pow', 'dex', 'app', 'siz', 'int', 'edu'] as const).map(key => (
              <div key={key} className="bg-slate-950 p-3 rounded-lg border border-slate-800 text-center relative group">
                ...
                <div className="mt-1">
                  <input
                    ...
                    className="w-16 text-center text-xl font-bold bg-transparent text-emerald-400 focus:outline-none focus:ring-1 focus:ring-emerald-500 rounded"
                  />
                </div>
                {char.edition === '7th' && (
                  <div className="text-[10px] text-slate-400 mt-1">
                    ½: {Math.floor(char.abilities[key] / 2)} | ⅕: {Math.floor(char.abilities[key] / 5)}
                  </div>
                )}
              </div>
            ))}
          ...
          <div className="pt-2 border-t border-slate-800/80">
            <div className="grid grid-cols-3 sm:grid-cols-6 gap-2 text-center text-xs">
              <div className="bg-slate-950 p-2 rounded-lg border border-slate-800">
                <span className="text-slate-400 block">HP (耐久)</span>
                <span className="text-base font-bold text-red-400">{char.hp} / {char.maxHp}</span>
              </div>
              <div className="bg-slate-950 p-2 rounded-lg border border-slate-800">
                <span className="text-slate-400 block">MP (マジック)</span>
                <span className="text-base font-bold text-blue-400">{char.mp} / {char.maxMp}</span>
              </div>
              <div className="bg-slate-950 p-2 rounded-lg border border-slate-800">
                <span className="text-slate-400 block">SAN (正気度)</span>
                <span className="text-base font-bold text-purple-400">{char.san}</span>
              </div>
              <div className="bg-slate-950 p-2 rounded-lg border border-slate-800">
                <span className="text-slate-400 block">ダメージボーナス</span>
                <span className="text-base font-bold text-amber-400">{char.damageBonus}</span>
              </div>
              <div className="bg-slate-950 p-2 rounded-lg border border-slate-800">
                <span className="text-slate-400 block">幸運</span>
                <span className="text-base font-bold text-emerald-400">{char.luck}</span>
              </div>
              <div className="bg-slate-950 p-2 rounded-lg border border-slate-800">
                <span className="text-slate-400 block">{char.edition === '7th' ? 'ビルド' : 'アイデア'}</span>
        ...
        <div className="bg-slate-900/60 p-5 rounded-xl border border-slate-800 space-y-4">
          ...
          <div className="space-y-3">
            <div className="bg-slate-950 p-3 rounded-lg border border-slate-800">
              <div className="flex justify-between text-xs text-slate-300 mb-1">
                <span>職業技能P</span>
                <span className={remainingJobPoints < 0 ? 'text-red-400 font-bold' : 'text-emerald-400'}>
                  残り {remainingJobPoints} / {maxJobPoints}
                </span>
              </div>
            ...
            <div className="bg-slate-950 p-3 rounded-lg border border-slate-800">
              <div className="flex justify-between text-xs text-slate-300 mb-1">
                <span>趣味・興味技能P</span>
                <span className={remainingHobbyPoints < 0 ? 'text-red-400 font-bold' : 'text-emerald-400'}>
                  残り {remainingHobbyPoints} / {maxHobbyPoints}
                </span>
              </div>
              <div className="w-full bg-slate-800 rounded-full h-2 overflow-hidden">
                <div
          ...
          <div className="text-xs text-slate-400 leading-relaxed border-t border-slate-800 pt-3">
            <p className="flex items-center gap-1.5 text-slate-300">
              ...
              <span>
                {char.edition === '7th'
                  ? '7版: 職業P = EDU×4 / 興味P = INT×2'
                  : '6版: 職業P = EDU×20 / 興味P = INT×10'}
              </span>
            </p>
          </div>
        </div>
      ...
      <div className="bg-slate-900/60 p-5 rounded-xl border border-slate-800 space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-slate-800">
          <input
            type="text"
            placeholder="オリジナルの技能名（例: 操縦(船舶)、芸術(写真)）"
            value={newSkillName}
            onChange={e => setNewSkillName(e.target.value)}
            className="px-3 py-1.5 bg-slate-950 border border-slate-800 rounded-lg text-xs text-slate-200 focus:outline-none focus:border-emerald-500 flex-1 min-w-[200px]"
          />
          <input
            type="number"
            placeholder="初期値"
            ...
            className="w-16 px-2 py-1.5 bg-slate-950 border border-slate-800 rounded-lg text-xs text-slate-200 text-center focus:outline-none focus:border-emerald-500"
          />
          <button
      ...
      <div className="bg-slate-900/60 p-5 rounded-xl border border-slate-800 space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-bold uppercase tracking-wider text-emerald-400 flex items-center gap-2">
            ...
            <span>バックストーリー ＆ ランダムジェネレーター</span>
          </h3>
          <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
          {[
            ...
            <div key={item.key} className="bg-slate-950 p-3 rounded-lg border border-slate-800 space-y-2">
              <div className="flex items-center justify-between">
              ...
              <textarea
                ...
                className="w-full bg-transparent text-slate-200 border-0 focus:outline-none resize-none placeholder-slate-600 leading-relaxed"
              />
            </div>
          ))}
        </div>
        ...
        <div className="bg-slate-950 p-3 rounded-lg border border-slate-800 space-y-1">
          <label className="block text-xs font-semibold text-slate-300">探索者メモ・持ち物・呪文など</label>
          <textarea
            rows={4}
            value={char.memo}
            onChange={e => setChar({ ...char, memo: e.target.value })}
            placeholder="所持品、遭遇した神話生物、習得した呪文、経歴などをご自由に記入してください"
